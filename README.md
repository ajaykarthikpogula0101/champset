...
The only paid dependency in the whole product is the LLM provider (OpenRouter).
...
Three roles, all routed through OpenRouter, all changeable in the app at **Settings → Models** (per user, no restart) or as defaults in `.env`:
